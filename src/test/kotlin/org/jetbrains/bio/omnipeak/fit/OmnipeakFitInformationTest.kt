...
class OmnipeakFitInformationTest {
    @get:Rule
  ...
  "fit.information.fqn": "${OmnipeakAnalyzeFitInformation::class.qualifiedName}",
  "version": 5
    ...
    fun checkWrongVersion() {
        ...
        expectedEx.expectMessage("expects '${OmnipeakAnalyzeFitInformation.VERSION}' version, but got '100500'")
        withTempFile("foo", ".tar") { path ->
    ...
    fun checkWrongFqn() {
        withTempFile("foo", ".tar") { path ->
            ...
            expectedEx.expectMessage(
                "Cannot load class org.jetbrains.bio.omnipeak.fit.Span100500FitInformation"
            )
            path.bufferedWriter().use {
                it.write(
                    """{
  ...
  "fit.information.fqn": "org.jetbrains.bio.omnipeak.fit.Span100500FitInformation",
  "version": 3
}"""
                )
            }
